...
export default function GeminiKeyForm({ hasCurrentKey }: GeminiKeyFormProps) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setErrorMsg("");
    try {
      const res = await fetch("/api/settings/geminikey", {
        method: "POST",
      ...
      setStatus("error");
      setErrorMsg(err instanceof Error ? err.message : "저장 중 오류");
    }
  }
  ...
  async function handleDelete() {
    ...
    setStatus("saving");
    try {
      await fetch("/api/settings/geminikey", { method: "DELETE" });
      setStatus("idle");
      router.refresh();
    } catch {
      setStatus("error");
  ...
  return (
    <form onSubmit={handleSave} className="space-y-3">
      <label className="block text-sm font-medium text-slate-700">Gemini API 키 입력</label>
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="AIza... 형식의 Gemini API 키를 붙여넣으세요"
      ...
      <p className="text-xs text-slate-400">키는 브라우저 쿠키에 저장되며, 이 기기에서만 유효합니다.</p>
    </form>
  );
}
