...
export default async function SettingsPage() {
  ...
  const keyPreview = cookieKey
    ? `${cookieKey.slice(0, 4)}****${cookieKey.slice(-4)}`
    : null;
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-600 mt-1 text-sm">
          KIPRIS API 키 등 서비스 연동 설정을 관리합니다.
        </p>
      </div>
      ...
      <div className="card p-6 space-y-5">
        ...
        {hasEnvKey && !hasCookieKey && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
            ...
            아래에서 다른 키를 입력하면 이 설정이 우선 적용됩니다.
          </div>
        )}
      ...
      <div className="card p-5">
        ...
        <div className="bg-slate-900 text-green-400 rounded-lg p-4 text-xs font-mono">
          <p className="text-slate-400"># Vercel Dashboard → Settings → Environment Variables</p>
          <p>KIPRIS_API_KEY=<span className="text-yellow-300">여기에_API_키_입력</span></p>
          <p className="mt-2 text-slate-400"># 또는 로컬 .env.local</p>
          <p>KIPRIS_API_KEY=<span className="text-yellow-300">여기에_API_키_입력</span></p>
        </div>
      </div>
    </div>
